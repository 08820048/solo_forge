...
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/admin/pricing-plans/${encodeURIComponent(planId)}`, {
      method: 'DELETE',
      headers: {
    ...
    if (!response.ok || !json?.success) {
      return NextResponse.json<ApiResponse<null>>(
        { success: false, message: json?.message || 'Failed to delete pricing plan' },
