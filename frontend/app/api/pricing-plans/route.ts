...
/**
 * GET
 * 读取前台定价方案列表（后端：GET /api/pricing-plans）。
 */
export async function GET(request: NextRequest) {
  try {
    const backendUrl = `${BACKEND_API_URL}/pricing-plans`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: data?.message || 'Failed to fetch pricing plans' },
