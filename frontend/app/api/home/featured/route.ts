...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/home/featured?${params.toString()}`, {
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ success: false, message: data?.message || 'Failed to fetch featured products' }, { status: response.status });
    }
